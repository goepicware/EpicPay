...
import { apiUrl, unquieID, showLoaderLst, hideLoaderLst } from "../Settings/Config";
...
class Topup extends Component {
  ...
  componentWillReceiveProps(PropsDt) {
    ...
    if(this.state.customerData !== PropsDt.customerdetails) {
        ...
        let avbl_credits = 0;
        if((Object.keys(PropsDt.customerdetails).length > 0) && (PropsDt.customerdetails.common_setting != undefined) && (Object.keys(PropsDt.customerdetails.common_setting).length > 0)) {
          tax_type = PropsDt.customerdetails.common_setting.company_tax_type;
          if(PropsDt.customerdetails.common_setting.company_tax_type == 'Exclusive') {
            tax_percentage = PropsDt.customerdetails.common_setting.company_tax_percentage;
            tax_percentage = (tax_percentage != undefined && tax_percentage != '') ? tax_percentage : 0;
          } else {
            tax_percentage = 0;
          }
        ...
        this.setState({ customerData: PropsDt.customerdetails, avbl_credits: avbl_credits, tax_type: tax_type, tax_percentage: tax_percentage });
    }
  }

  selectedTopupPlan(topupplan) {
    this.setState({ selectedplan: topupplan }, function () {
  ...
  topupplanListing() {
    ...
    if(Object.keys(topupplanList).length > 0) {
      const topupplanHtml = topupplanList.map((topupplan, rwInt) => {
        let msnCls = ((Object.keys(selectedplan).length > 0) && (selectedplan.topupplan_id == topupplan.topupplan_id)) ? 'active' : '';
        return (<li className={msnCls} onClick={this.selectedTopupPlan.bind(this,topupplan)}>
  ...
  render() {
    ...
    return (
      <div className="main-div">
        ...
        {(Object.keys(selectedplan).length > 0) && <IonFooter collapse="fade">
          <div className="sticky-single-btn">
            <a href="#" className="button " onClick={this.purchasePlan.bind(this,selectedplan)}>

...
class voucherDetail extends Component {
  constructor(props) {
    ...
    let slugValue = (typeof props.match.params.slugValue !== "undefined" ? props.match.params.slugValue : "");
    console.log('slugValue', slugValue);
    if(slugValue != '' && slugValue != undefined) {
        this.props.getProductList("&product_type=5&product_slug="+slugValue);
    } else {
        props.history.push("/");
    }
  }
  ...
  render() {
    ...
    if(Object.keys(productList).length > 0) {
      let proImg = (productList[0].product_thumbnail != '') ? productList[0].product_thumbnail : noImage;
      let proName = (productList[0].product_alias != '') ? productList[0].product_alias : productList[0].product_name;
      let productPrice = (productList[0].product_price != '') ? parseFloat(productList[0].product_price) : 0;
      let productInfo = (productList[0].product_long_description !== '') ? Parser(stripslashes(productList[0].product_long_description)) : '';
            ...
            <div className="voucher-detail">
              <div className="voucher-detail-header">
                ...
                <div className="point-coin">
                  ...
                  <span>Valid Till {productList[0].product_voucher_expiry_datetxt}</span>
                </div>
              </div>
        ...
        <IonFooter collapse="fade">
            <div className="sticky-redeem">
                <div className="sticky-redeem-bg">
                  <span>
                    <img src={coin} /> Balance : <strong>{customerAvailablePoints}</strong>
                  </span>
                  {(parseFloat(customerAvailablePoints) < productPrice)?<a href="javascrip:void(0)" className="button low-balance-cls">
                    Low Balance
                  </a>:<a href="#" className="button " onClick={this.redeemNow.bind(this, productList[0].product_primary_id)}>
                    Redeem Now
                  </a>}
                </div>
            </div>
...
export default connect(mapStateTopProps, mapDispatchToProps)(withRouter(voucherDetail));

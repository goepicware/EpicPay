...
class Vouchers extends Component {
  constructor(props) {
    ...
    var vouchersShow = cookie.load("vouchers_show") != "" && cookie.load("vouchers_show") != undefined ? cookie.load("vouchers_show") : "all";
  ...
  componentWillReceiveProps(PropsDt) {
    ...
    if(this.state.promotionlist !== PropsDt.promotionlist) {
        ...
        if(PropsDt.promotionlist != '' && PropsDt.promotionlist != undefined && (Object.keys(PropsDt.promotionlist).length > 0)) {
            availablePromo = PropsDt.promotionlist.my_promo;
            redeemedPromo = PropsDt.promotionlist.promo_history;
            availablePromoCount = Object.keys(PropsDt.promotionlist.my_promo).length;
        }
        this.setState({ promotionlist: PropsDt.promotionlist, available_promo_count: availablePromoCount, available_promo: availablePromo, redeemed_promo: redeemedPromo});
    }
  }
  ...
  allVouchersList() {
    ...
    if(Object.keys(productList).length > 0) {
      ...
      const productListHtml = productList.map((product, rwInt) => {
            let msnCls = '';
            let proImg = (product.product_thumbnail != '') ? product.product_thumbnail : noImage;
            let proName = (product.product_alias != '') ? product.product_alias : product.product_name;
            let productPrice = (product.product_price != '') ? parseFloat(product.product_price) : 0;
            return (<li>
              <figure>
                <img src={proImg} />
              </figure>
              <figcaption>
                ...
                <Link to={"/vouchers/"+product.product_slug} title="Vouchers View" className="button">
                  Redeem
                </Link>
              </figcaption>
            </li>);
  ...
  gotoMyVoucher(voucherData, event) {
    event.preventDefault();
    let voucherDataTxt = (Object.keys(voucherData).length > 0)?JSON.stringify(voucherData):'';
  ...
  availableVouchersList() {
    ...
    if(Object.keys(availablePromoLst).length > 0) {
      ...
      const availablePromoListHtml = availablePromoLst.map((availablePromo, rwInt) => {
            let msnCls = '';
            let proImg = (availablePromo.promotion_image != '') ? availablePromo.promotion_image : noImage;
            let proName = (availablePromo.promotion_created_from == 'Cron') ? availablePromo.promotion_desc : availablePromo.promo_code;
            let productPrice = (availablePromo.promotion_max_amt != '') ? availablePromo.promotion_max_amt : 0;
            return (<li>
              <figure>
                <img src={proImg} />
              </figure>
              <figcaption>
                ...
                {(availablePromo.promotion_created_from != 'Cron') && <span>
                  {productPrice} <img src={coin} />
                </span>}
                <Link onClick={this.gotoMyVoucher.bind(this,availablePromo)} title="Vouchers View" className="button">
                  Redeem
                </Link>
              </figcaption>
            </li>);
  ...
  redeemedVouchersList() {
    ...
    if(Object.keys(redeemedPromoLst).length > 0) {
      ...
      const redeemedPromoListHtml = redeemedPromoLst.map((redeemedPromo, rwInt) => {
            let msnCls = '';
            let proImg = (redeemedPromo.promotion_image != '') ? redeemedPromo.promotion_image : noImage;
            let proName = (redeemedPromo.promotion_created_from == 'Cron') ? redeemedPromo.promotion_desc : redeemedPromo.promo_code;
            let productPrice = (redeemedPromo.promotion_max_amt != '') ? redeemedPromo.promotion_max_amt : 0;
            return (<li>
              <figure>
                <img src={proImg} />
              </figure>
              <figcaption>
                ...
                {(redeemedPromo.promotion_created_from != 'Cron') && <span>
                  {productPrice} <img src={coin} />
                </span>}
              </figcaption>
            </li>);
          ...
          <div className="container">
            <div className="vouchers-list textcenter">
              <div className="vouchers-nav">
                <ul>
                  <li className={(activetab == 'all')?"active":""} onClick={this.nevTabFun.bind(this,'all')}>
                    <a href="#">All</a>{" "}
                  </li>
                  <li className={(activetab == 'available')?"active":""} onClick={this.nevTabFun.bind(this,'available')}>
                    <a href="#">Available ({this.state.available_promo_count})</a>{" "}
                  </li>
                  <li className={(activetab == 'redeemed')?"active":""} onClick={this.nevTabFun.bind(this,'redeemed')}>
                    <a href="#">Redeemed</a>{" "}
                  </li>
                </ul>
              </div>
...
export default connect(mapStateTopProps, mapDispatchToProps)(withRouter(Vouchers));

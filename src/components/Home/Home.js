...
class Home extends Component {
  constructor(props) {
    ...
    var refcode = (localStorage.getItem('refcode') === null) ? '' : localStorage.getItem('refcode');
    var triggerOTP = 'No';
    var triggerFrom = '';
  ...
  handleFldChange(event) {
    ...
    } else if (event.target.name === "reg_otp_dgt1" || event.target.name === "reg_otp_dgt2" || event.target.name === "reg_otp_dgt3" || event.target.name === "reg_otp_dgt4") {
      if (event.target.value === "" || re.test(event.target.value)) {
        var mblnumber = this.space(event.target.value);
        var mblnumberLenght = mblnumber.length;
        if(mblnumberLenght <= 1) {
          ...
          this.setState(update(this.state, { regotpdata: { [field]: { $set: fieldvalue } } }),
          function () {
  ...
  triggerOTPFunction() {
    ...
    if(regphoneNo != '') {
      ...
      axios.post(apiUrl + "customer/send_otponly", qs.stringify(postObject))
      .then((res) => {
        hideLoaderLst('home-page-maindiv','class');
        if(res.data.status === "ok") {
          this.setState({ bottompopup: 'regotp', runingNum: 120 },
                function () {
                  this.setRuningNum();
                }.bind(this)
          );
  ...
  regSendOtp(typeflg, event) {
    ...
    if(regphoneNo != '') {
      ...
      axios.post(apiUrl + "customer/send_customer_otp", qs.stringify(postObject))
      .then((res) => {
        hideLoaderLst(loadrdivcls,'class');
        if(res.data.status === "ok") {
          if((typeflg == 'login') || (typeflg == 'register' && res.data.is_existing_user == 'yes')) {
            let custArr = res.data.result;
            cookie.save("UserId", custArr.customer_id);
            cookie.save("UserEmail", custArr.customer_email);
            cookie.save("UserFname", custArr.customer_first_name !== "" ? custArr.customer_first_name : "");
            cookie.save("UserLname", custArr.customer_last_name !== "" ? custArr.customer_last_name : "");
  ...
  displayRunNumbar() {
      if(this.state.regotp_error != '') {
        ...
        return(<p className="runing_num">Resend in {this.state.runingNum} Seconds</p>);
      } else {
  ...
  cancelAct(event) {
    ...
    var triggerfrom = this.state.trigger_from;
        triggerfrom = (triggerfrom != '' && triggerfrom != undefined) ? triggerfrom : 'myaccount';
  ...
  tgrVerifyOtp(event) {
    ...
    var regotpval = regotpData.reg_otp_dgt1+regotpData.reg_otp_dgt2+regotpData.reg_otp_dgt3+regotpData.reg_otp_dgt4;
  ...
  if(regphoneNo != '' && errorMgs == '') {
      ...
      axios.post(apiUrl + "customer/customer_otp_verification", qs.stringify(postObject))
      .then((res) => {
  ...
  goBackTo(refPage,event) {
    event.preventDefault();
    var regotpdata = { reg_otp_dgt1: '', reg_otp_dgt2: '', reg_otp_dgt3: '', reg_otp_dgt4: '' }; 
    this.setState({ bottompopup: refPage, reg_phone_no: '', regphone_error: '', regotpdata: regotpdata, regotp_terms: 'No', regotp_promo: 'No', regotp_error: '', cust_name: '', cust_email_id: '', birthday_month: '', cust_ref_code: '', regcust_error: ''});
  }
      ...
      <div className="main-div home-page-maindiv">
        ...
        <BottomSheet open={true}>
          <div className="container">
            ...
            {(this.state.bottompopup == 'regphone') && <div className="welcome-phone">
              ...
              <div className="input-phone">
                <div className="prefix-merge">
                  ...
                  <div className="nxt-fix">
                    <input type="input" inputMode='numeric' pattern="[0-9]*" name="reg_phone_no" value={this.state.reg_phone_no} placeholder="Phone number" onChange={this.handleFldChange.bind(this)} />
                  </div>
                </div>
                {(this.state.regphone_error != '') && <span className="error-info">{this.state.regphone_error}</span>}
                <p>
              ...
              <div className="bottom-btn bb-txt textcenter">
                <a href="#" className="button full-btn reg-otp-link" onClick={this.regSendOtp.bind(this,'register')}>
                  Send OTP
                </a>
                <p>
                  Do you have an account ? <a href="#" onClick={this.goBackTo.bind(this,'loginfrm')}>Login here</a>
                </p>
              </div>
            </div>}

            {(this.state.bottompopup == 'confirm') && <div className="update-personal">
              <div className="up-header confirm-header textcenter">
                <h2>You have entered<br></br>+65 {this.state.reg_phone_no}</h2>
                <img src={cellphone1} />
              </div>
              <div className="up-form confirm-phone">
                 <p>A one-time verification code will be sent to this mobile number</p>
                ...
                <div className="up-frm-btn">
                  <a href="#" className="button ghost-btn" onClick={this.goBackTo.bind(this,'regphone')}>
                    Edit
                  </a>
                  <a href="#" className="button cust-reg-btn" onClick={this.confirmPhone.bind(this)}>
                    Confirm
                  </a>
                </div>
              </div>
            ...
            {(this.state.bottompopup == 'regotp') && <div className="enter-otp">
              <h2>Enter OTP</h2>
              <div className="four-bx-col">
                <div className="four-bx">
                  <input type="input" inputMode='numeric' pattern="[0-9]*" name="reg_otp_dgt1" id="reg_otp_dgt1" value={this.state.regotpdata.reg_otp_dgt1} onChange={this.handleFldChange.bind(this)} />
                  <input type="input" inputMode='numeric' pattern="[0-9]*" name="reg_otp_dgt2" id="reg_otp_dgt2" value={this.state.regotpdata.reg_otp_dgt2} onChange={this.handleFldChange.bind(this)} />
                  <input type="input" inputMode='numeric' pattern="[0-9]*" name="reg_otp_dgt3" id="reg_otp_dgt3" value={this.state.regotpdata.reg_otp_dgt3} onChange={this.handleFldChange.bind(this)} />
                  <input type="input" inputMode='numeric' pattern="[0-9]*" name="reg_otp_dgt4" id="reg_otp_dgt4" value={this.state.regotpdata.reg_otp_dgt4} onChange={this.handleFldChange.bind(this)} />
                </div>
                {this.displayRunNumbar()}
              </div>
              {(this.state.trigger_otp != 'Yes') && <div className="otp-checkbox">
                <ul>
                  <li>
                    <input type="checkbox" name="regotp_terms" onChange={this.changeRegOtpChkBox.bind(this,"trams")}
                  ...
                  <li>
                    <input type="checkbox" name="regotp_promo" onChange={this.changeRegOtpChkBox.bind(this,"promo")}
              ...
              {(this.state.trigger_otp === 'Yes') ? <div className="up-frm-btn">
                  <a href="#" className="button ghost-btn" onClick={this.cancelAct.bind(this)}>
                    Cancel
                  </a>
                  <a href="#" className="button cust-reg-btn" onClick={this.tgrVerifyOtp.bind(this)}>
                    Continue
                  </a>
                </div> : <div className="bottom-btn textcenter">
                <a href="#" className="button full-btn reg-otp-verify" onClick={this.regVerifyOtp.bind(this)}>
                  Continue
                </a>
              </div>}
            </div>}

            {(this.state.bottompopup == 'regpersonal') && <div className="update-personal">
              <div className="up-header textcenter">
                <p>Update your personal details</p>
              </div>
              <div className="up-form">
                <input type="text" placeholder="Name" name="cust_name" value={this.state.cust_name} onChange={this.handleFldChange.bind(this)} />
                <input type="text" placeholder="Email Address" name="cust_email_id" value={this.state.cust_email_id} onChange={this.handleFldChange.bind(this)} />
                <select name="birthday_month" value={this.state.birthday_month} className="components_selct components_selctbox_cls" onChange={this.handleFldChange.bind(this)}  >
                ...
                <input type="text" placeholder="Referal Code" name="cust_ref_code" value={this.state.cust_ref_code} onChange={this.handleFldChange.bind(this)} />
                {(this.state.regcust_error != '') && <p className="error_info">{this.state.regcust_error}</p>}
                <div className="up-frm-btn">
                  <a href="#" className="button ghost-btn" onClick={this.goBackTo.bind(this,'regphone')}>
                    Cancel
                  </a>
                  <a href="#" className="button cust-reg-btn" onClick={this.custRegistration.bind(this)}>
                    Register
                  </a>
                </div>
              </div>
            ...
            {(this.state.bottompopup == 'loginfrm') && <div className="welcome-phone login-frm">
              <div className="wp-top">
              ...
              <div className="input-phone">
                ...
                {(this.state.regphone_error != '') && <span className="error-info">{this.state.regphone_error}</span>}
                <p>
                  We will send you a one-time SMS, Additional carriers charges may apply
                </p>
              </div>
              <div className="bottom-btn bb-txt textcenter">
                <a href="#" className="button full-btn login-otp-link" onClick={this.regSendOtp.bind(this,'login')}>
                  Login
                </a>
                <p>
                  Do not have an account ? <a href="#" onClick={this.goBackTo.bind(this,'regphone')}>Register here</a>
                </p>
              </div>
            </div>}
            ...
            {(this.state.bottompopup == 'loginotp') && <div className="enter-otp enter-otp-login">
              ...
              <div className="bottom-btn">
                <a href="#" className="button full-btn" onClick={this.loginVerifyOtp.bind(this)}>
                  Login
                </a>
              </div>
            </div>}
